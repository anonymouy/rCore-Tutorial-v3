...
// WARMUP must be >= NIC RX ring depth (16 for virtio-net, 32 for e1000)
// so stale echo replies left in the ring by a prior run are consumed
// before the measurement window opens. Kernel-side drain is not possible
// because virtio-net's recv() blocks on empty. See bypass_setup().
...
///
/// When `seq` is `Some` and `payload_size >= 4`, the first 4 bytes of the
/// payload carry the sequence number in big-endian form. RTT uses this to
/// match replies and discard stale packets. TX-only and burst benchmarks
/// pass `None`, leaving the payload as pure 0xAB fill so a selective echo
/// server can ignore them consistently with `bench_e1000.rs`.
...
fn parse_ipv4_udp(pkt: &[u8]) -> Option<(u16, u16, usize, usize)> {
    if pkt.len() < 34 || u16::from_be_bytes([pkt[12], pkt[13]]) != 0x0800 || pkt[23] != 17 {
        return None;
    }
    ...
    let udp_len = u16::from_be_bytes([pkt[udp_start + 4], pkt[udp_start + 5]]) as usize;
    if udp_len < 8 {
        return None;
    }
    let payload_start = udp_start + 8;
    let payload_end = (udp_start + udp_len).min(pkt.len());
    Some((src_port, dst_port, payload_start, payload_end))
}
...
unsafe fn tx_slot_commit(hdr: *mut NetBypassHeader, slot: *mut u8, frame_len: usize, tx_head: u32) {
    ...
    while sent < count {
        ...
        if let Some((slot, tx_head)) = acquired {
            unsafe {
                ...
                let frame_len = build_udp_frame_into(
                    slot_buf,
                    mac,
                    ip,
                    SRC_PORT,
...
unsafe fn bench_rtt_latency(
    ...
    // Seq-matching the reply needs ≥4 bytes of payload; the bench's
    // smallest size is 64, so this is informational only.
    if payload_size < 4 {
        println!(
            "  [skip] payload_size {} < 4, can't seq-match",
            payload_size
        );
        return;
    }
    ...
    // Cap how many stale frames we'll consume before giving up on a
    // single request, so a wedged network doesn't hang the bench.
    const MAX_DISCARDS_PER_REQ: usize = 32;

    for i in 0..total {
        // Make seq distinct across payload sizes too: a stale 64-byte
        // echo from the previous size's run won't match the current
        // 512-byte run's seq even at the same iteration index.
        let seq: u32 = ((payload_size as u32) << 16) | (i as u32 & 0xFFFF);
        ...
        for _ in 0..MAX_DISCARDS_PER_REQ + 1 {
            ...
            // Classify for diagnostic output (every received frame).
            if pkt_len >= 34 {
            ...
            // Match request to reply: must be UDP from DST_PORT to
            // SRC_PORT and carry our seq number in the first 4 payload
            // bytes. Anything else is stale and gets dropped.
            let is_my_reply = match parse_ipv4_udp(pkt) {
                Some((src_port, dst_port, payload_start, payload_end)) => {
                    src_port == DST_PORT
                        && dst_port == SRC_PORT
